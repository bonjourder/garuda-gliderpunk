...
{
	pipeSprite = new Sprite("Assets/Gambar/MELLO.png");
	pipeSprite->SetScale(Vector3(1,1,1));
}
...
Pipe::Pipe()
{
	if (!pipeSprite)
	{
		cout << "Mencoba instansiasi pipe" << endl;
		return;
	}
...
void Pipe::Render()
{
	topSprite.Render();
	botSprite.Render();
	topRB.Render(Vector3(0, 0, 0));
	botRB.Render(Vector3(0, 0, 0));
...
BadanKaku Pipe::GetTopBK()
{
	return topRB;
}

BadanKaku Pipe::GetBotBK()
{
	return botRB;
}