...
void Sprite::Render()
{
	...
	glBegin(GL_QUADS);
	{
		...
		glTexCoord2f(1, 0);		glVertex2i((float)texture.GetWidth(), 0);
		glTexCoord2f(1, 1);		glVertex2i((float)texture.GetWidth(), (float)texture.GetHeight());
		glTexCoord2f(0, 1);		glVertex2i(0, (float)texture.GetHeight()); */
	}
	glEnd();
